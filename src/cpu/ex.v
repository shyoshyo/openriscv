...
module ex(
	...
	always @ (*)
		if(rst_n == `RstEnable)
			...
			case (aluop_i)
				`EXE_SLL_OP: shiftout <= reg2_i << reg1_i[4:0];
				`EXE_SRL_OP: shiftout <= reg2_i >> reg1_i[4:0];
				`EXE_SRA_OP: shiftout <= $signed(reg2_i) >>> reg1_i[4:0];
	...
	assign overflow_sum =
			(!reg1_i[31] && !reg2_i_mux[31] &&  result_sum[31]) || ( reg1_i[31] &&  reg2_i_mux[31] && !result_sum[31]);
	...
	always @ (*)
		...
		begin
			...
			if((aluop_i == `EXE_ADD_OP || aluop_i == `EXE_ADDI_OP || aluop_i == `EXE_SUB_OP) && overflow_sum)
			begin
				wreg_o <= `WriteDisable;
				ovassert <= `OverflowAssert;
			end
			else
			begin
				wreg_o <= wreg_i;
